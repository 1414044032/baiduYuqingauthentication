...
class BceSigner(object):
    ...
    def normalized_uri(uri):
        ...
        """
        return quote(BceSigner.get_utf8_value(uri), safe='-_.~/')
    ...
    def normalized(msg):
        ...
        """
        return quote(BceSigner.get_utf8_value(msg), safe='-_.~')

    def _calc_signingkey(self, auth):
        """ Get a a signing key """
        print(auth)
        string_to_sign = "/".join((auth['version'], auth['access'].decode(),
...
def serialize_authorization(auth):
    ...
    val = "/".join((auth['version'], auth['access'].decode("utf-8"), auth['timestamp'], auth['period'],
